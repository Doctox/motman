-- LA FENÊTRE DE LECTURE PASSE À TRENTE SECONDES — 19/09/2026
--
-- Demande du propriétaire, la veille de la sortie Play : quinze secondes ne
-- suffisaient toujours pas pour lire la grille avant le premier tour. Le tour
-- n°1 ne court donc pour personne pendant trente secondes, et le premier joueur
-- garde ses quarante-cinq secondes entières derrière : 30 + 45 = 75.
--
-- Même partage que les fois précédentes : les parties NORMALES et le défi tiennent
-- leurs horodatages de la fonction edge (`matchSetup.ts`, FIRST_TURN_READING_MS),
-- les parties CLASSÉES démarrent ici, en plpgsql. `src/matchTiming.test.ts`
-- échoue si l'une bouge sans l'autre.
--
-- La fonction est recopiée telle quelle depuis 20260917160000, à ces deux
-- intervalles près.
...
begin
  ...
  update public.server_matches
  set status = 'active',
      turn_started_at = pg_catalog.clock_timestamp() + interval '30 seconds',
      turn_ends_at = pg_catalog.clock_timestamp() + interval '75 seconds',
      updated_at = pg_catalog.clock_timestamp()
  where id = ready.match_id
    and status = 'pending'
