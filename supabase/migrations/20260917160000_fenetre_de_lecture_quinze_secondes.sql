-- LA FENÊTRE DE LECTURE PASSE À QUINZE SECONDES — 17/09/2026
--
-- Dix secondes ne suffisaient pas pour lire une grille entière : demande du
-- propriétaire après l'avoir essayée sur son téléphone. Le tour n°1 ne court
-- donc pour personne pendant quinze secondes, et le premier joueur garde ses
-- quarante-cinq secondes entières derrière : 15 + 45 = 60.
--
-- Même règle que la migration du même jour qui l'avait posée à dix : les parties
-- NORMALES tiennent leurs horodatages de la fonction edge
-- (`matchSetup.ts`, FIRST_TURN_READING_MS), les parties CLASSÉES démarrent ici,
-- en plpgsql, quand les deux joueurs ont accepté. `matchTiming.test.ts` tient
-- les deux d'accord, et il échoue si l'une bouge sans l'autre.
--
...
begin
  ...
  update public.server_matches
  set status = 'active',
      turn_started_at = pg_catalog.clock_timestamp() + interval '15 seconds',
      turn_ends_at = pg_catalog.clock_timestamp() + interval '60 seconds',
      updated_at = pg_catalog.clock_timestamp()
  where id = ready.match_id
    and status = 'pending'
