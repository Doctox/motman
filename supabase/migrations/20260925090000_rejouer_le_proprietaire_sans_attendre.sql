-- ─────────────────────────────────────────────────────────────────────────────────
-- ON PEUT REJOUER LE PROPRIÉTAIRE TOUT DE SUITE (25/09/2026)
--
-- Le matchmaking classé écarte un candidat contre qui on vient de finir une
-- partie classée depuis moins de dix minutes. La règle empêche deux joueurs de
-- se renvoyer la balle en boucle pour se fabriquer du classement.
--
-- Elle a un effet pervers à cette échelle : quand le propriétaire reçoit
-- l'alerte qu'une recherche classée s'est ouverte et vient jouer, le joueur
-- qu'il vient d'affronter se retrouve seul pour dix minutes. La règle le
-- protège d'un abus qui n'existe pas, et le prive de la seule partie
-- disponible.
--
-- Elle saute donc pour toute paire dont un côté est `admin`, et reste entière
-- entre deux joueurs ordinaires.
--
-- La fonction est recopiée de la migration 20260908120000 à l'identique : seule
-- la clause du rematch change.
-- ─────────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  where search.status = 'searching'
    ...
    and (
      -- EXEMPTION DU PROPRIÉTAIRE (25/09/2026). « Enlève cette règle, comme ça
      -- si quelqu'un veut spammer les classées contre moi il peut. »
      --
      -- À cette échelle, la pause protège surtout du vide : celui qui vient de
      -- jouer contre lui n'a plus personne, et repart pour dix minutes
      -- d'animation sans adversaire. Le propriétaire campe dans la file pour
      -- donner des parties — il doit pouvoir en enchaîner.
      --
      -- La règle reste entière entre deux joueurs ordinaires : c'est là qu'elle
      -- sert, à empêcher un duo de se fabriquer du classement en boucle.
      exists (
        select 1
        from public.profiles as exempte
        where exempte.id in (p_user_id, search.user_id)
          and exempte.role = 'admin'
      )
      or
      not exists (
        select 1
